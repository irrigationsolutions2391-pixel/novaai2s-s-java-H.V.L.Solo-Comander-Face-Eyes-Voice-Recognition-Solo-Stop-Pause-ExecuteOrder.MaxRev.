...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { Novaai2JavInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw Novaai2JavInvalidDataException("`$name` is not set")
            is JsonNull -> throw Novaai2JavInvalidDataException("`$name` is null")
            else -> throw Novaai2JavInvalidDataException("`$name` is invalid, received $this")
        }
